...
class Model(nn.Module):
    ...
    def save(self, model_path, weights_only=False):
        model_state_dict = self.state_dict() if not self.n_gpus > 1 else self.module.state_dict()
        if weights_only:
            if self.using_tpu:
    ...
    def load(self, model_path, weights_only=False, device="cuda"):
        ...
        if weights_only:
            self.load_state_dict(model_dict) if not self.n_gpus > 1 else self.module.load_state_dict(model_dict) 
        else:
            self.load_state_dict(model_dict["state_dict"]) if not self.n_gpus > 1 else self.module.load_state_dict(model_dict["state_dict"]) 
