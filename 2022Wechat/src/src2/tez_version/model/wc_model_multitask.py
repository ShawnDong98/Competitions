...
class WeChatModel(nn.Module):
    def __init__(
        ...
        self.features_attn = EmbeddingAttention(
            hidden_size=vlad_hidden_size + bert_output_size * output_features_cnt,
        )
    ...
    def forward(self, 
        ...
        features_list = self.features_attn(features_list)

        final_features = self.fusion(features_list)

        final_features_attn = self.embedding_attn(final_features)
        logits = self.classifier(final_features_attn)
        outputs['cls'] = logits
