...
class HSIDataSet(DetDataset):
    ...
    def parse_dataset(self):
        ...
        if 'annotations' not in coco.dataset:
            ...
            logger.warning('Annotation file: {} does not contains ground truth and load image information only.'.format(anno_path))
        
        len_img_ids = len(img_ids)
        for idx, img_id in enumerate(img_ids[:self.sample_num]):
            ...
            logger.info(f"Loading {idx}/{self.sample_num if self.sample_num != -1 else len_img_ids} HSI from {im_path}...")
            hsi = sio.loadmat(im_path)['dest'].astype(np.float32)
            coco_rec = {
                'image': hsi, 
                'im_file': im_path,
