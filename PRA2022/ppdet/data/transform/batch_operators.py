...
class BatchRandomResize(BaseOperator):
    ...
    def __call__(self, samples, context=None):
        ...
        return resizer(samples, context)


@register_op
class BatchRandomResizeHSI(BaseOperator):
    """
    Resize image to target size randomly. random target_size and interpolation method
    Args:
        target_size (int, list, tuple): image target size, if random size is True, must be list or tuple
    ...
    def __init__(self,
                 ...
                 random_size=True,
                 random_interp=False):
        super(BatchRandomResizeHSI, self).__init__()
        self.keep_ratio = keep_ratio
        self.interps = [
            cv2.INTER_NEAREST,
            cv2.INTER_LINEAR,
            # cv2.INTER_AREA, # only 4 channel 
            cv2.INTER_CUBIC,
            cv2.INTER_LANCZOS4,
        ]
        self.interp = interp
...
class Gt2YoloTarget(BaseOperator):
    ...
    def __call__(self, samples, context=None):
        ...
        an_hw = np.array(self.anchors) / np.array([[w, h]]) # 将 anchor 归一化
        for sample in samples:
            gt_bbox = sample['gt_bbox']
            gt_class = sample['gt_class']
            if 'gt_score' not in sample:
                sample['gt_score'] = np.ones(
                    (gt_bbox.shape[0], 1), dtype=np.float32) # 如果没有 score， 就全赋值为 1
            ...
            for i, (mask, downsample_ratio) in enumerate(zip(self.anchor_masks, self.downsample_ratios)):
                ...
                target = np.zeros(
                    ...
                    dtype=np.float32) # (3, 6 + n_cls, grid_h, grid_w)
