...
class YOLOv3Loss(nn.Layer):
    ...
    def __init__(self,
        ...
        if self.loss_weights != None:
            self.loss_weight = paddle.to_tensor(loss_weight, dtype="float32")

    def obj_loss(self, pbox, gbox, pobj, tobj, anchor, downsample):
        """
        pbox: [(batch, 3, 13, 13, 1)] * 4
        gbox: [(batch, 3, 13, 13, 1)] * 4
        pobj: (batch, 3, 13, 13, 1)
        tobj: (batch, 3, 13, 13, 1)
        anchor: (anchor_h, anchor_w)
        """
        # pbox
        pbox = decode_yolo(pbox, anchor, downsample)
        pbox = xywh2xyxy(pbox)
        ...
        # gbox
        gxy = gbox[:, :, 0:2] - gbox[:, :, 2:4] * 0.5 # x1 = cx - 1/2 * w, y1 = cy - 1/2 * h
        gwh = gbox[:, :, 0:2] + gbox[:, :, 2:4] * 0.5 # x2 = cx + 1/2 * w, y2 = cy + 1/2 * h
    ...
    def cls_loss(self, pcls, tcls):
        ...
        loss_cls = F.binary_cross_entropy_with_logits(
            pcls, tcls, reduction='none', pos_weight=self.loss_weight)
        # loss_cls = F.binary_cross_entropy_with_logits(
        #     pcls, tcls, reduction='none')
        # loss_cls = F.sigmoid_focal_loss(pcls, tcls, alpha=1.0, gamma=2.0, reduction='none')
        return loss_cls

    def yolov3_loss(self, p, t, gt_box, anchor, downsample, scale=1., eps=1e-10):
        """
        p: (batch, 30, height/down_ratio, width/down_ratio)
        t: (batch, 3, 11, height/down_ratio, width/down_ratio)
        gt_box: (batch, 50, 4)
        na: 3
        anchor:  (anchor_h, anchor_w)
        """
        ...
        p = p.reshape((b, na, -1, h, w)).transpose((0, 1, 3, 4, 2)) #(b, 3, 10, 13, 13) -> (b, 3, 13, 13, 10) 10分别表示: x, y, w, h, obj, [cls onehot]
        x, y = p[:, :, :, :, 0:1], p[:, :, :, :, 1:2]
        w, h = p[:, :, :, :, 2:3], p[:, :, :, :, 3:4]
        obj, pcls = p[:, :, :, :, 4:5], p[:, :, :, :, 5:]
        self.distill_pairs.append([x, y, w, h, obj, pcls])
        # print("1 pcls.shape: ", pcls.shape)

        t = t.transpose((0, 1, 3, 4, 2))  # (b, 3, 11, 13, 13) -> (b, 3, 13, 13, 11) 11分别表示: tx, ty, tw, th, tscale, tobj, [tcls onehot]
        tx, ty = t[:, :, :, :, 0:1], t[:, :, :, :, 1:2]
        tw, th = t[:, :, :, :, 2:3], t[:, :, :, :, 3:4]
        tscale = t[:, :, :, :, 4:5]
        tobj, tcls = t[:, :, :, :, 5:6], t[:, :, :, :, 6:]
        # print("1 tcls.shape", tcls.shape)
        ...
        loss_xy = loss_xy.sum([1, 2, 3, 4]).mean() # 对除了 batch 之外的元素求和求平均

        # print("w: ", paddle.mean(w))
        # print("tw: ", paddle.mean(tw))
        # print("h: ", paddle.mean(h))
        # print("th: ", paddle.mean(th))
    ...
    def forward(self, inputs, targets, anchors):
        """
        inputs: [(batch, 30, height/down_ratio, width/down_ratio)] * 3
        gt_targets: [(batch, 3, 11, height/down_ratio, width/down_ratio)] * 3
        """
        np = len(inputs)
