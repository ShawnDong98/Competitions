...
def get_infer_results(outs, catid, bias=0):
    ...
    if 'mask' in outs:
        # mask post process
        infer_res['mask'] = get_seg_res(outs['mask'], outs['bbox'], outs['bbox_num'], im_id, catid)
    ...
    if classwise:
        ...
        for idx, catId in enumerate(cat_ids):
            ...
            if precision.size:
                ...
                for i in range(pr_array.size - 1, 0, -1):
                    pr_array[i - 1] = np.maximum(pr_array[i - 1], pr_array[i])  #[1.     1.     0.6666 0.4285 0.3043 0.    ]
                # to calculate area under PR curve, look for points
                # where X axis (recall) changes value
                i = np.where(recall_array[1:] != recall_array[:-1])[0]  #precision前后两个值不一样的点

                # AP= AP1 + AP2+ AP3+ AP4
                ap = np.sum((recall_array[i + 1] - recall_array[i]) * pr_array[i + 1])
            else:
                ap = float("nan")
