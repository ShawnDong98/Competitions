...
@register
@serializable
class CustomImageFolder(DetDataset):
    def __init__(self,
                 sample_num=-1,
                 **kwargs):
        super(CustomImageFolder, self).__init__(sample_num=sample_num)
        self._imid2path = {}
        self.roidbs = None
        self.sample_num = sample_num
    ...
    def _load_images(self):
        ...
        for image in images:
            assert image != '' and os.path.isfile(image), \
                    "Image {} not found".format(image)
            im_id = int(image.split('/')[-1][:-4])
            rec = {'im_id': np.array([im_id]), 'im_file': image}
            self._imid2path[im_id] = image
            records.append(rec)
        assert len(records) > 0, "No image file found"
        return records
