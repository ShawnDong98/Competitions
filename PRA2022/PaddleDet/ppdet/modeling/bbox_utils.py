...
def decode_yolo(box, anchor, downsample_ratio):
    ...
    x1 = (x + grid[:, :, :, :, 0:1]) / grid_w # x 方向偏移量
    y1 = (y + grid[:, :, :, :, 1:2]) / grid_h # y 方向偏移量

    anchor = paddle.to_tensor(anchor)
    anchor = paddle.cast(anchor, x.dtype)
    anchor = anchor.reshape((1, na, 1, 1, 2))
    w1 = paddle.exp(w) * anchor[:, :, :, :, 0:1] / (downsample_ratio * grid_w) # anchor 仅用于计算 w 和 h
    h1 = paddle.exp(h) * anchor[:, :, :, :, 1:2] / (downsample_ratio * grid_h) # 相当于在每个 grid cell 有 3 个 anchor
