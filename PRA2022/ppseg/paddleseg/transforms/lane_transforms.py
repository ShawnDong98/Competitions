...
class SubImgCrop:
    ...
    def __call__(self, data):
        ...
        if self.offset_top > 0 and self.offset_top < data['img'].shape[0]:
            data['img'] = data['img'][self.offset_top:, :, :]
            if data['label'] is not None:
                data['label'] = data['label'][self.offset_top:, :]

        if self.offset_bottom > 0 and self.offset_bottom < data['img'].shape[0]:
            data['img'] = data['img'][:-self.offset_bottom, :, :]
            if data['label'] is not None:
                data['label'] = data['label'][:-self.offset_bottom, :]

        if self.offset_left > 0 and self.offset_left < data['img'].shape[1]:
            data['img'] = data['img'][:, self.offset_left:, :]
            if data['label'] is not None:
                data['label'] = data['label'][:, self.left_w_off:]

        if self.offset_right > 0 and self.offset_right < data['img'].shape[1]:
            data['img'] = data['img'][:, :-self.offset_right, :]
            if data['label'] is not None:
                data['label'] = data['label'][:, :-self.offset_right]
